...
- pub.dev 배포 워크플로 수정 마무리. 1.3.0·1.3.1 은 CI 문제로 pub.dev 에 실리지
  못했습니다. **1.3.0 을 쓰려던 분은 1.3.2 를 쓰면 됩니다 — 코드는 같습니다.**

## 1.3.1

- **pub.dev 배포 수정** — 1.3.0 은 태그까지 올라갔지만 pub.dev 에 실리지
  못했습니다. `dart-lang/setup-dart` 의 재사용 워크플로가 OIDC 로 받은
  `PUB_TOKEN` 을 의존성 해석 단계에까지 걸어두는데, 그 토큰은 이 패키지를
  발행할 권한만 갖기 때문에 pub.dev 가 `test`·`http`·`crypto` 조회를 403 으로
  막았습니다. 해석 단계에서만 토큰을 비우도록 워크플로를 직접 작성했습니다.
  **코드 변경은 없습니다 — 1.3.0 을 쓰려던 분은 1.3.1 을 쓰면 됩니다.**
- 1.3.0 이후 사실이 아니게 된 주석 정정 — 휴대폰 발신번호는 PASS 대신 신분증
  사본을 첨부해 접수할 수 있고, 카카오 채널 인증번호도 여러분 화면에서
  끝납니다.
